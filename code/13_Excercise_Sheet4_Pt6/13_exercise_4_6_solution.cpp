...
int main ()
{
	...
	// solution using brackets for powers -> not minimal:
	//   r4 = 1/x + 2/(x*x) + 3/(x*x*x) + 4/(x*x*x*x);
	// with "*=" operator -> complex to read but more generic:
	//   float xd = x;
	//   r4 = 1/xd + 2/(xd*=x) + 3/(xd*=x) + 4/(xd*=x);
